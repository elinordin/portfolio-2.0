const projects = [
    {
        title: 'Internago',
        image: '../assets/graphics/internago.png',
        whatIDid: ['Design', 'Frontend', 'Team Lead'],
        techStack: ['React', 'Gatsby', 'Figma'],
        readMe: 'https://github.com/elinordin/Habit-tracker/blob/main/README.md',
        url: 'https://inspiring-heyrovsky-bd5ba4.netlify.app/'
    },
    {
        title: 'Trime',
        image: '../assets/graphics/trime.png',
        whatIDid: ['Scrum', 'Frontend', 'Navigation'],
        techStack: ['React Native', 'Figma', 'NPM'],
        readMe: 'https://github.com/trimeTeam/Trime-App/blob/main/README.md',
    },    
    {
        title: 'Live Github-Stats',
        image: '../assets/graphics/github.png',
        totalCommits: '162',
        mostUsedLanguages: ['HTML', 'JavaScript', 'CSS', 'PHP', 'EJS'],
        readMe: 'https://github.com/elinordin',
    },
    {
        title: 'Secrets',
        image: '../assets/graphics/secrets.png',
        whatIDid: ['Backend', 'Frontend', 'Database'],
        techStack: ['MongoDB', 'Node', 'Express'],
        readMe: 'https://github.com/elinordin/secrets/blob/main/README.md',
        url: 'https://elinordin.github.io/secrets/'
    },
    {
        title: 'Doosti',
        image: '../assets/graphics/doosti.png',
        whatIDid: ['Design', 'Frontend', 'NUF'],
        techStack: ['Figma', 'CSS', 'JavaScript'],
        readMe: 'https://github.com/elinordin/doosti/blob/main/README.md',
        url: 'https://elinordin.github.io/doosti/'
    },
    {
        title: 'Rota',
        image: '../assets/graphics/rota.png',
        whatIDid: ['Frontend', 'Mob coding', 'Design'],
        techStack: ['Figma', 'JavaScript', 'CSS'],
        readMe: 'https://github.com/elinordin/Habit-tracker/blob/main/README.md',
        url: 'https://elinordin.github.io/habit-tracker/'
    }
]

export default projects;